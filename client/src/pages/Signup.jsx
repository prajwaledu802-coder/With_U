...
import { signUp, signInWithGoogle, syncWithBackend } from '../services/authService';
      ...
      if (result.session) {
        const trustedContact =
          form.contactName && form.contactEmail
            ...
            : null;
        await syncWithBackend({ name: form.name, ...(trustedContact ? { trustedContact } : {}) });
        toast.success('Welcome to WITH_U');
        navigate('/dashboard');
      } else {
        ...
        >
          <GlassCard strong>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <details className="rounded-2xl bg-white/30 dark:bg-white/5 p-4 mt-2">
                <summary className="cursor-pointer text-sm font-medium flex items-center gap-2">
                  <UserPlus size={14} className="text-warm-500" />
                  {t('auth.trustedContact')}
                </summary>
              ...
              <button disabled={busy} className="btn-primary w-full mt-2" id="signup-submit">
            ...
            <button onClick={google} className="btn-ghost w-full" id="signup-google">
            ...
            <p className="text-sm text-center mt-6 opacity-60">
              {t('auth.haveAccount')}{' '}
              <Link to="/login" className="font-medium text-warm-500 hover:underline underline-offset-4">
                {t('auth.switchLogin')}
              </Link>
            </p>
          </GlassCard>
